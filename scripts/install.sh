...
# both from source with a specific Swoole ./configure flag set — see the
# phlix-server repo's docker/Dockerfile.base and docker/README.md ("Swoole
# build flags"). On bare metal (apt-based Debian/Ubuntu) we mirror that exact
# build here.
#
...
# Debian -dev packages mirroring the Alpine set in Dockerfile.base. php-dev
...
phlix_build_swoole() {
  ...
  (
    cd "$tmp/swoole"
    phpize
    # Flags copied verbatim from the phlix-server docker/Dockerfile.base — do
    # not change them here; change them there (the source of truth) and
    # re-sync. --enable-iouring / --enable-uring-socket build on any kernel
    # but only activate at RUNTIME on Linux kernel >= 5.6; older kernels
    # silently fall back to epoll.
    ./configure \
      --enable-swoole \
      --enable-sockets \
      --enable-mysqlnd \
...
#
# The sysctl drop-in is byte-identical to the one phlix-server writes, but each
# installer owns its own filename — the two services are commonly co-located on
# one box, and a shared file would be deleted by whichever uninstall ran first.
#
...
# unit template in section 6 and the `--update` retrofit in do_update).
PHLIX_SYSCTL_FILE="/etc/sysctl.d/99-phlix-hub-net.conf"
PHLIX_LIMITS_FILE="/etc/security/limits.d/99-phlix-hub.conf"
...
# the chown/permission steps around the call site and moving it there would
...
# Managed by phlix-hub scripts/install.sh — rewritten on every --update.
...
# Ephemeral port range for OUTBOUND sockets.
#
...
# to bind later. It also keeps every port the hub listens on (2206, 8800,
# 8802-8805) outside the ephemeral range by construction — no
...
SYSCTL_EOF
  ...
  # PAM limits: covers LOGIN SHELLS and hand-run CLI scripts. It does NOT cover
  # the systemd service — that is LimitNOFILE= in the unit.
  log "Applying PAM nofile limits ($PHLIX_LIMITS_FILE)"
  mkdir -p "$(dirname "$PHLIX_LIMITS_FILE")"
  cat > "$PHLIX_LIMITS_FILE" <<LIMITS_EOF
# Managed by phlix-hub scripts/install.sh — rewritten on every --update.
#
# PAM-only: applies to login shells and CLI invocations, NOT to the phlix-hub
# systemd unit (systemd never reads limits.conf — the daemon's ceiling is
# LimitNOFILE= in /etc/systemd/system/phlix-hub.service).
#
...
do_uninstall() {
  ...
  # phlix-server may be co-installed and still relying on the same knobs (it
  # ships its own identically-valued drop-in, so its file survives this).
...
ExecStartPre=/usr/bin/env bash ${INSTALL_PATH}/scripts/wait-for-free-ports.sh -t 90 ${gate_hub_port} 8802 8803 8804 8805 2206" "$SERVICE_FILE"
  ...
  # 5a-2. Kernel/resource tuning (Workerman kernel-optimization appendix). Two
  ...
  #       inherits systemd's soft 1024, and the relay surfaces (:8802/:8803)
  #       run on count=1 workers holding one socket per paired server and per
  #       connected client — the hub hits "Too many open files" and stops
  #       accepting tunnels well before any other resource runs out.
  #
  ...
  # ExecStartPre= would also match a bare '^ExecStart' prefix, so the fallback
  # anchor is '^ExecStart=' with the '=' — inserting above ExecStartPre would
  # still be valid systemd, but the '=' keeps the placement predictable.
  #
  # Resolve the real service user from the unit: an older install may run as
  # www-data, and the PAM drop-in must name whoever the unit actually uses
  # rather than this script's default. Passed as an argument — NOT by
  # reassigning $SERVICE_USER, which the surrounding chown/permission steps
  # also read.
  local unit_user=""
...
# actually gets (default soft 1024 / hard 524288). The count=1 relay workers\\
# hold one socket per paired server and per connected client.\\
...
# Kernel/resource tuning (Workerman kernel-optimization appendix). Runs here
# because the PAM drop-in names $SERVICE_USER, so the account must already
# exist; must also land before the unit is written in section 6. Also
# re-applied by do_update.
phlix_apply_kernel_tuning
...
# (what MySQL reports when reverse-DNS resolves 127.0.0.1 → localhost
# via /etc/hosts, which is the default Ubuntu behaviour). Without the
# 'localhost' grant, PDO connections fail with "Access denied for user
# '${DB_USER}'@'localhost'" even though the configured host is 127.0.0.1.
mysql <<SQL
...
# live connection. That matters more here than anywhere else in the stack: the
# relay surfaces (:8802 servers, :8803 clients) run on count=1 workers, so ONE
# process holds a socket for EVERY paired server and every connected client
# simultaneously. At soft 1024 the hub stops accepting tunnels with "Too many
# open files" long before any other resource is exhausted.
LimitNOFILE=1048576
