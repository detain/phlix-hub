 ...
 * strings `@phlix/ui` renders — against the REAL tag-pinned `@phlix/ui` bundle,
 ...
 *
 * Mirrors the proven phlix-tizen-client tests/unit/i18n.test.ts doctrine and the
 * phlix-server web-ui twin of this suite.
 *
...
describe('end-to-end: client messages override reaches ui-rendered strings (real @phlix/ui, no mocks)', () => {
    ...
    it('omitting messages renders the untouched English default (behavior-preservation pin)', async () => {
        const baseline = createPhlixApp({ app: 'hub', apiBase: 'http://phlix.invalid' });
    ...
    it('the en boot (empty override) renders byte-identical English defaults', async () => {
        ...
        // ui English defaults at the pinned tag — any drift here is a ui-side
