 ...
 *
 * ## The seam this wires (verified against the resolved tag pin)
 *
 ...
 * hub's web-ui passed no `messages`, so the seam existed but was unreachable
 ...
 * `@phlix/ui` pin (the `v0.99.5` release tag) SHIPS the locale bundles
 ...
 *
 * 1. Bump the `@phlix/ui` pin to a tag whose `LOCALE_MESSAGES` includes the
 *    tag.
