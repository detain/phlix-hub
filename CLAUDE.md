...
**Domain** (`src/Hub/`): claim/enroll/heartbeat/relay/sharing handlers + `EnrollmentJwtService` (Ed25519), `Ed25519KeyManager`, `RelaySessionManager`, `DnsAliasManager`, `TlsCertificateManager`, DTOs (`LibraryShare`, `InviteLink`, `SharedLibraryDto`). **Relay** (`src/Relay/`): `RelayWorker` (:8802 servers) · `ClientRelayWorker` (:8803 clients) · `Tunnel`/`TunnelManager` · `Frame{Encoder,Decoder}`. **Federation** (`src/Federation/`). **Auth** (`src/Auth/`): `AuthManager` · `JwtHandler` (HS256) · `UserRepository`.

**DB**: `src/Common/Database/` — `PhlixMySQLConnection`, `ConnectionPool`, `MigrationRunner`. Logging: `src/Common/Logger/LoggerFactory::get(LogChannels::*)`. SSR: Smarty via `src/Common/WebPortal/PageRenderer.php` → `public/templates/{auth,home,layouts,partials}/`, hydrated by `public/assets/js/*.js`.
