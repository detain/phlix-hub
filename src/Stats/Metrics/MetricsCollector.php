 ...
 *
 * This is the single object injected into the request / connection hooks and
 * shared with {@see MetricsFlushService}. Every recording method no-ops when
 * the subsystem is disabled (`config.metrics.enabled = false`) so the hot
 * request path carries zero overhead when metrics are off.
 *
