...
/**
 * Read side of the metrics subsystem (consumed by the admin controller in S4).
 *
 * Uses named `:param` placeholders (hub convention) and resolves the MySQL
 * connection via the static {@see ConnectionPool} rather than constructor injection.
 *
 * Every query is SELECT-leading and aggregates with SUM / GROUP BY.
 *
...
final class MetricsRepository implements MetricsRepositoryInterface
{
    ...
    /**
     * Current hub-wide snapshot over the trailing window.
     *
    ...
    public function history(int $minutes = 60, int $resolutionSeconds = 60): array
    {
        ...
        // `FLOOR(UNIX_TIMESTAMP(bucket_started_at) / :res)` expression. Under
        // sql_mode=ONLY_FULL_GROUP_BY (MySQL 8 default) the checker does not
        // recognise the bucket column buried inside the SELECT's
        // FROM_UNIXTIME(FLOOR(...) * :res) as functionally dependent on that inner
        // GROUP BY expression, so it rejects the whole query with error 1055 and
        // the admin metrics endpoint 500s. Grouping by the alias makes the grouped
        // value IS the selected value, which satisfies the dependency check.
        $rows = $this->select(
            "SELECT
                 ...
                 COALESCE(SUM(h_gt_5000), 0) AS h_gt_5000
             FROM metrics_rollup
             WHERE bucket_started_at >= (NOW() - INTERVAL :minutes MINUTE)
             GROUP BY bucket
             ORDER BY bucket ASC",
            [
