...
namespace Phlix\Hub\Common\Http;

/**
 * Trusted-proxy-aware client-IP resolver (SV-4.15 HIGH fix, ported to the hub).
 *
 * Every IP-keyed rate limiter (login / jwks / client-mount WS) needs the REAL
 * client address, not one an attacker can forge. The naive "leftmost
 * `X-Forwarded-For` entry" is fully client-controlled: HAProxy's
 * `option forwardfor` APPENDS the connecting address to whatever the client
 * sent, so a forged leftmost value survives and every request lands in a fresh
 * rate-limit bucket — defeating the limiter entirely.
 *
 ...
 *    This is the safe default when the hub is exposed directly (dev / misconfig).
 ...
 *    are never reached.
 * 3. If no untrusted XFF hop is present, the proxy-set `X-Real-IP` (HAProxy sets
 *    it from the connecting address, so it is not client-spoofable) is used.
 * 4. Failing all of that, the sanitised peer address is returned.
 *
 * ## Deployment contract (why the default is loopback-only)
 *
 * The hub is fronted by HAProxy over loopback: HTTP → `127.0.0.1:8800` and the
 * client relay WS → `127.0.0.1:8803` (both carrying `option forwardfor`), while
 * the server tunnel on `:8802` is a DIRECT TLS listener (NOT proxied). For the
 * proxied surfaces the peer the hub observes is therefore always `127.0.0.1`,
 * and HAProxy appends the real client as the RIGHTMOST XFF entry. The default
 * trusted set is therefore LOOPBACK ONLY. Adding RFC1918 ranges by default would
 * be actively wrong for this topology: a client on the same LAN (e.g.
 * `192.168.1.50`) would be treated as a proxy, skipped, and the NEXT-left
 * (client-forged) XFF entry would be trusted instead — reopening the spoof. When
 * a NON-loopback proxy fronts the hub, add its address/CIDR via the bare
 * `TRUSTED_PROXIES` env (comma-separated IPs and/or CIDRs).
 *
 * The resolver ALWAYS returns a value validated as a well-formed IP (≤45 chars
 * for IPv6), or a hard-truncated fallback, so a forged/oversized header can never
 * produce a key longer than the rate-limit bucket key's storage bound.
 *
 * @package Phlix\Hub\Common\Http
 */
final class TrustedProxyResolver
{
    /**
     * Default trusted proxies: loopback only. Matches the shipped HAProxy
     * deployment, where the front connects to the hub over `127.0.0.1` / `::1`.
     *
     * @var list<string>
     */
    ...
    public function resolve(string $remoteIp, ?string $forwardedFor, ?string $realIp = null): string
    {
        ...
        // No untrusted XFF hop: fall back to the proxy-set X-Real-IP (HAProxy sets
        // it from the connecting address, so it is not client-spoofable).
        if ($realIp !== null) {
     ...
     * an unexpected/malformed peer value can never overflow the key storage bound.
     */
