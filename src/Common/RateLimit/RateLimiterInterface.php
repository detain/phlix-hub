 ...
 *
 * The contract is deliberately backend-agnostic: the Phase-0 default
 * binding is the worker-local in-memory {@see RateLimiter}, but a
 * cluster-safe implementation (a shared DB table or shared memory) can be
 * swapped in later (Feature #6) without touching call sites — callers
 * depend only on this interface and the {@see RateLimitState} value
 * object.
 *
