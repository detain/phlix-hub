 ...
 * is evicted. This is the fix for finding B1 — the previous
 * `private static array $rateLimitStore` in {@see \Phlix\Hub\Auth\AuthManager}
 * grew without bound (one entry per distinct IP, never reclaimed in a
 * resident-memory worker), a slow memory leak. There is no `static`
 * state here; the map is an instance field with a hard size ceiling.
 *
 ...
 * acceptable for the single-box hub deployment. A cluster-safe backend
 * (shared DB table / shared memory) can be substituted later (Feature #6)
 * by binding a different {@see RateLimiterInterface} implementation — call
