...
/**
 * `user:delete {user} [--force] [--json]` — permanently remove a hub account.
 *
 * Two distinct gates, deliberately ordered:
 *
 *  1. The last-admin invariant, read straight off the repository's public
 *     {@see UserRepository::countAdmins()}, is checked FIRST and is absolute:
 ...
 * every refusal emits the shared `{"ok":false,"error":"..."}` shape. The
 * {@see UserRepository} is resolved lazily through the injected factory.
 *
 * @package Phlix\Hub\Console\Commands
 */
#[AsCommand(name: 'user:delete', description: 'Delete a hub user account (last admin is protected)')]
final class UserDeleteCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        try {
            ...
            if (self::isLastAdmin($repository, $user)) {
