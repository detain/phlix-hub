 ...
 * with `--revoke`, demotes them. Demotion is gated by the last-admin invariant
 * read straight off the repository's public {@see UserRepository::countAdmins()}:
 * the final administrator cannot be demoted, mirroring the hub's HTTP admin
 * surface. Promotion needs no guard — it can only ever add an administrator.
 *
 * (The server sibling routes this check through a repository `isLastAdmin()`
 * helper; the hub deliberately has no such method, so the predicate lives here
 * rather than adding API to `UserRepository` that nothing else would use.)
 *
 * With `--json` the resulting admin flag is emitted under the shared success
 * envelope. The {@see UserRepository} is resolved lazily through the injected
 * factory so constructing this command never opens a database connection.
 *
 * @package Phlix\Hub\Console\Commands
 */
#[AsCommand(name: 'user:promote', description: 'Promote a hub user to admin, or demote with --revoke')]
final class UserPromoteCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        try {
            ...
            if ($revoke && self::isLastAdmin($repository, $user)) {
    ...
    /**
     * The last-admin predicate, read directly off the public count so the hub
     * needs no repository change. True only when the target row is itself an
     * admin AND no other admin exists.
     *
     * @param array<string, mixed> $user
     */
    private static function isLastAdmin(UserRepository $repository, array $user): bool
    {
        if (empty($user['is_admin'])) {
            return false;
        }

        return $repository->countAdmins() <= 1;
    }
