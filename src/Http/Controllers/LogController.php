...
/**
 * Admin log viewer — lists the hub log files and tails them.
 *
 * Mirrors phlix-server's admin log viewer so operators get the same "watch the
 * logs" experience on the hub. Mounted behind the auth + admin middleware, so
 * no per-action access check is needed here.
 *
 *  - `GET /api/v1/me/logs` — list `*.log` files in the configured log dir
 *    (name, size, modified time).
 *  - `GET /api/v1/me/logs/tail?file=app.log&lines=N` — last N lines of one file.
 *  - `GET /api/v1/me/logs/tail-all?lines=N` — last N lines across *every*
...
final class LogController
{
    ...
    {
        $rawFile = $this->queryString($request, 'file');
     ...
     */
    public function tailAll(Request $request): Response
    {
        $lines = $this->queryLines($request);
    ...
    /**
     * Read and clamp the `lines` query parameter to [1, MAX_LINES].
     */
    private function queryLines(Request $request): int
    {
        /** @var array<string, mixed> $query */
        $query = $request->query;
        /** @var mixed $raw */
