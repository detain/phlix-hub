 ...
 * GET driven entirely by the worker's event loop (S75).
 *
 * ## Why this shape
 *
 * `Client::get($url, $success, $error)` with BOTH callbacks supplied never
 * enters `Client::request()`'s coroutine branch (`!isset($options['success'])
 * && Coroutine::isCoroutine()` — vendor `Client.php:78`). The request is
 * queued, the socket is driven by the Workerman event loop, and the callback
 * fires later on that same loop. So:
 *
 ...
 *  - the caller never waits on a `Swoole\Coroutine\Channel`, which would be a
 *    hard error outside a coroutine.
 *
 * The {@see Client} is created LAZILY on first fetch and then reused: its
 * constructor builds a connection pool, and one pooled client per hub process
 * is bounded (it is not a per-request object, so it is not a resident-memory
 * leak).
 *
