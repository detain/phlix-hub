...
/**
 * S187 — the whole-tree unused-import guard, hub leg.
 *
 * Port of the server-side guard shipped at detain/phlix-server a5cde27e (PR #744,
 * tests/Unit/Support/UnusedImportGuardTest.php). Per ruling R4 this is a COPY, not
 * a re-create: the scanner (scanSource / parseImportNames / phpFiles / depth
 * accounting) is algorithm-identical to the server original — verified by running
 * both over the same corpus (server control read exactly 0/1781). Only the
 * hub-idiom surroundings differ, and none of them changes a verdict:
 *
 *  - namespace + doc header are hub style;
 *  - GUARDED_ROOTS is derived from the hub tree (see the constant's comment);
 *  - the corpus floor matches the hub estate size, not the server's;
 *  - the mutation fixtures name real hub files instead of server files;
 *  - SURVIVAL_TOKEN is this lane's own token.
 *
 * Every `use` import in the guarded PHP estate (all `*.php` below each guarded root
 * except `vendor/`, `node_modules/` and `.git` segments, plus the top-level
 * `*.php` files) must be referenced somewhere in its own file outside its import
 * statement. This gate exists because the S187 prep probe measured 66 such imports
 * accumulating silently across src/, tests/ and scripts/ alone before any fix:
 * no CI leg checked (the vendored phpcs has no Namespaces sniffs and composer.json
 * cannot churn — option (b)), so the invariant now lives as a phpunit gate and
 * rides the existing PHPUnit step, exactly as it does on the server.
 *
 * Conservative by design: a short name that appears ONLY in a docblock or comment
 * still counts as USED. PHPStan level 9 and Psalm resolve FQCNs inside docblocks
 * through these imports, and inline suppressions lean on them; deleting such an
 * import would redden or silently mis-resolve the analyser legs. The gate
 * therefore reports strictly fewer imports than a token-only analyser would —
 * never more than are safe to delete.
 *
 * The mutation tests below prove detection rather than argue it: they copy real
 * src/ and tests/ fixtures into a temp tree, plant a genuinely unused import, and
 * require this scanner to report it red. An empty tree is a scanner wiring
 * failure, not a pass (S345 floor rule): phpFiles() throws instead of returning
 * an empty list, so a mis-wired scan can never pass vacuously.
 */
final class UnusedImportGuardTest extends TestCase
{
    /** Lane survival token (S187 hub). Code-resident by design; never prose. */
    public const string SURVIVAL_TOKEN = 'S187HUBGUARDX6T2';

    /** Path segments excluded from the estate — identical to the server guard. */
    private const EXCLUDED_SEGMENTS = ['vendor', 'node_modules', '.git'];

    /**
     * Roots whose estate must be unused-import-free at all times.
     *
     * Derived from the hub tree at 2f07019b (server list was
     * src,tests,scripts,config,migrations,examples):
     *  - 'examples' — no such root exists in the hub;
     *  - 'migrations' — the hub root carries only .sql files (0 PHP at merge
     *    time; `phpFiles()` fail-fasts on a PHP-free root by design, so listing
     *    it would throw on every run). Re-add 'migrations' if it ever carries
     *    PHP again — same accounting the server made when it dropped 'public'.
     * Top-level *.php (start.php) is covered by its own test below.
     */
    private const GUARDED_ROOTS = ['src', 'tests', 'scripts', 'config'];
    ...
    public function testEveryImportInTheEstateIsReferencedInItsFile(): void
    {
        ...
        $this->assertGreaterThan(400, $scanned, 'guard ran on a partial tree — estate missing?');
    ...
    {
        $row = $this->plantAndScan(\dirname(__DIR__, 3) . '/src/Auth/AuthManager.php', 'GhostKlassQq');
    ...
    {
        $fixture = \dirname(__DIR__, 3) . '/tests/Unit/Support/CrossRepoPathAssertCiWiringTest.php';
    ...
    {
        $fixture = \dirname(__DIR__, 3) . '/src/Auth/AuthManager.php';
    ...
    private static function phpFiles(string $dir): array
    {
        ...
        // PHPStan L9 types the traversal value of RecursiveIteratorIterator as
        // mixed; the instanceof guard is the hub house idiom (see
        // StaticAnalysisScopeTest::testTheTestsCorpusIsActuallyPopulated) —
        // default fileinfo mode yields SplFileInfo only, so the filter is
        // provably never-true-skips: identical file set, no verdict change (R4).
        foreach ($it as $file) {
    ...
    {
        $dir = sys_get_temp_dir() . '/s187-hub-guard-' . bin2hex(random_bytes(6));
