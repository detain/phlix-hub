 ...
 *
 * Exercises the three behaviours that matter for finding B1: TTL window
 * expiry (a stale window restarts rather than accumulating forever),
 * cap-bounded active eviction (the map never grows past the configured
 * ceiling, fixing the unbounded-static-map leak), and {@see RateLimiter::reset()}.
 * Time is driven by an injected clock so windows advance deterministically
 * with no real sleeps.
 */
