...
namespace Phlix\Hub\Tests\Unit\Common\Http;

use Phlix\Hub\Common\Http\TrustedProxyResolver;
use PHPUnit\Framework\TestCase;

/**
 * Trusted-proxy-aware client-IP resolution used to build spoof-resistant
...
final class TrustedProxyResolverTest extends TestCase
{
     ...
     * (HAProxy `option forwardfor` appends the connecting address). The rightmost
    ...
    /**
     * When XFF is absent, the proxy-set X-Real-IP (set by HAProxy, not
     ...
     * value longer than the IPv6 max (45 chars), protecting the bucket key bound.
     */
