 ...
 * `exports` map blocks subpath JSON access, and vendoring byte-identical
   ...
   * 2d11ac2a1140a58500c30019406fde09).
 *
...
final class S280ContractsRouteManifestParityTest extends TestCase
{
     ...
     */
    private const S280_SURVIVAL_TOKEN = 'S280hubgate-manifest-parity@402-12125138';

    /**
     * cs#40 lane ritual token: code-resident only, space-free, asserted
     * present in this gate (tokenized corpus) and absent from the data.
     */
    private const CS40_RITUAL_TOKEN = 'CS40CURRENCYPINX9W4';
    ...
    {
        $this->assertSame(19, strlen(self::CS40_RITUAL_TOKEN), 'cs#40 ritual token shape');
        $this->assertSame(
            self::CS40_RITUAL_TOKEN,
            (string) preg_replace('/[^A-Z0-9]/', '', self::CS40_RITUAL_TOKEN),
            'cs#40 ritual token must be upper-alphanumeric and space-free',
        );
        $this->assertStringNotContainsString(
            self::CS40_RITUAL_TOKEN,
            (string) file_get_contents(self::HUB_FIXTURE_PATH),
            'the cs#40 ritual token belongs to code, not to the vendored hub fixture',
        );
        $this->assertStringNotContainsString(
            self::CS40_RITUAL_TOKEN,
            (string) file_get_contents(self::CONTRACTS_EXPORT_PATH),
            'the cs#40 ritual token belongs to code, not to the vendored contracts export',
        );
        $this->assertStringNotContainsString(
            self::S280_SURVIVAL_TOKEN,
            (string) file_get_contents(self::HUB_FIXTURE_PATH),
