 ...
 * `exports` map blocks subpath JSON access, and vendoring byte-identical
   ...
   * 206bc4d4ba7f18177a2e41406fd33716).
 *
...
final class S280ContractsRouteManifestParityTest extends TestCase
{
     ...
     */
    private const S280_SURVIVAL_TOKEN = 'S280hubgate-manifest-parity@401-812ff0a7';

    /**
     * cs#33 lane ritual token: code-resident only, space-free, asserted
     * present in this gate (tokenized corpus) and absent from the data.
     */
    private const CS33_RITUAL_TOKEN = 'CS33CURRENCYPINX9W5';
    ...
    {
        $this->assertSame(19, strlen(self::CS33_RITUAL_TOKEN), 'cs#33 ritual token shape');
        $this->assertSame(
            self::CS33_RITUAL_TOKEN,
            (string) preg_replace('/[^A-Z0-9]/', '', self::CS33_RITUAL_TOKEN),
            'cs#33 ritual token must be upper-alphanumeric and space-free',
        );
        $this->assertStringNotContainsString(
            self::CS33_RITUAL_TOKEN,
            (string) file_get_contents(self::HUB_FIXTURE_PATH),
            'the cs#33 ritual token belongs to code, not to the vendored hub fixture',
        );
        $this->assertStringNotContainsString(
            self::CS33_RITUAL_TOKEN,
            (string) file_get_contents(self::CONTRACTS_EXPORT_PATH),
            'the cs#33 ritual token belongs to code, not to the vendored contracts export',
        );
        $this->assertStringNotContainsString(
            self::S280_SURVIVAL_TOKEN,
            (string) file_get_contents(self::HUB_FIXTURE_PATH),
