 ...
 * `exports` map blocks subpath JSON access, and vendoring byte-identical
   ...
   * 932bb005086ba4ae77270f5ec07ab1a6).
 *
...
final class S280ContractsRouteManifestParityTest extends TestCase
{
     ...
     */
    private const S280_SURVIVAL_TOKEN = 'S280hubgate-manifest-parity@402-925e77c1';

    /**
     * cs#38 lane ritual token: code-resident only, space-free, asserted
     * present in this gate (tokenized corpus) and absent from the data.
     */
    private const CS38_RITUAL_TOKEN = 'CS38CURRENCYPINX9W2';
    ...
    {
        $this->assertSame(19, strlen(self::CS38_RITUAL_TOKEN), 'cs#38 ritual token shape');
        $this->assertSame(
            self::CS38_RITUAL_TOKEN,
            (string) preg_replace('/[^A-Z0-9]/', '', self::CS38_RITUAL_TOKEN),
            'cs#38 ritual token must be upper-alphanumeric and space-free',
        );
        $this->assertStringNotContainsString(
            self::CS38_RITUAL_TOKEN,
            (string) file_get_contents(self::HUB_FIXTURE_PATH),
            'the cs#38 ritual token belongs to code, not to the vendored hub fixture',
        );
        $this->assertStringNotContainsString(
            self::CS38_RITUAL_TOKEN,
            (string) file_get_contents(self::CONTRACTS_EXPORT_PATH),
            'the cs#38 ritual token belongs to code, not to the vendored contracts export',
        );
        $this->assertStringNotContainsString(
            self::S280_SURVIVAL_TOKEN,
            (string) file_get_contents(self::HUB_FIXTURE_PATH),
