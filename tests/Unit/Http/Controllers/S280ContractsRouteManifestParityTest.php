 ...
 * `exports` map blocks subpath JSON access, and vendoring byte-identical
   ...
   * 6bcd16be8a0ed5fbab8a42121cf28a19).
 *
...
final class S280ContractsRouteManifestParityTest extends TestCase
{
     ...
     */
    private const S280_SURVIVAL_TOKEN = 'S280hubgate-manifest-parity@401-2c1a9662';

    /**
     * cs#32 lane ritual token: code-resident only, space-free, asserted
     * present in this gate (tokenized corpus) and absent from the data.
     */
    private const CS32_RITUAL_TOKEN = 'CS32CURRENCYPINX9W4';
    ...
    {
        $this->assertSame(19, strlen(self::CS32_RITUAL_TOKEN), 'cs#32 ritual token shape');
        $this->assertSame(
            self::CS32_RITUAL_TOKEN,
            (string) preg_replace('/[^A-Z0-9]/', '', self::CS32_RITUAL_TOKEN),
            'cs#32 ritual token must be upper-alphanumeric and space-free',
        );
        $this->assertStringNotContainsString(
            self::CS32_RITUAL_TOKEN,
            (string) file_get_contents(self::HUB_FIXTURE_PATH),
            'the cs#32 ritual token belongs to code, not to the vendored hub fixture',
        );
        $this->assertStringNotContainsString(
            self::CS32_RITUAL_TOKEN,
            (string) file_get_contents(self::CONTRACTS_EXPORT_PATH),
            'the cs#32 ritual token belongs to code, not to the vendored contracts export',
        );
        $this->assertStringNotContainsString(
            self::S280_SURVIVAL_TOKEN,
            (string) file_get_contents(self::HUB_FIXTURE_PATH),
