 ...
 * `exports` map blocks subpath JSON access, and vendoring byte-identical
   ...
   * 16a32ec5563a51d98b1fce55820fc119).
 *
...
final class S280ContractsRouteManifestParityTest extends TestCase
{
     ...
     */
    private const S280_SURVIVAL_TOKEN = 'S280hubgate-manifest-parity@402-eb6bb619';

    /**
     * cs#39 lane ritual token: code-resident only, space-free, asserted
     * present in this gate (tokenized corpus) and absent from the data.
     */
    private const CS39_RITUAL_TOKEN = 'CS39CURRENCYPINX9W3';
    ...
    {
        $this->assertSame(19, strlen(self::CS39_RITUAL_TOKEN), 'cs#39 ritual token shape');
        $this->assertSame(
            self::CS39_RITUAL_TOKEN,
            (string) preg_replace('/[^A-Z0-9]/', '', self::CS39_RITUAL_TOKEN),
            'cs#39 ritual token must be upper-alphanumeric and space-free',
        );
        $this->assertStringNotContainsString(
            self::CS39_RITUAL_TOKEN,
            (string) file_get_contents(self::HUB_FIXTURE_PATH),
            'the cs#39 ritual token belongs to code, not to the vendored hub fixture',
        );
        $this->assertStringNotContainsString(
            self::CS39_RITUAL_TOKEN,
            (string) file_get_contents(self::CONTRACTS_EXPORT_PATH),
            'the cs#39 ritual token belongs to code, not to the vendored contracts export',
        );
        $this->assertStringNotContainsString(
            self::S280_SURVIVAL_TOKEN,
            (string) file_get_contents(self::HUB_FIXTURE_PATH),
